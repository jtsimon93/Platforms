...
void Player::Init(const Vector2 spawnPosition) {
  ...
  sprite.SetOffset(
      {0, -(sprite.GetSize().y / 2) - 32 + 5}); // Adjust for sprite height
  ...
  // Snap the player to the ground initially
  SnapPlayerToGround();
}
...
void Player::HandleInput() {
  if (onGround && (IsKeyPressed(KEY_SPACE) || IsKeyPressed(KEY_W) ||
                   IsKeyPressed(KEY_UP))) {
...
void Player::CollisionCheck(const TiledMap *map) { GroundCheck(map); }

void Player::GroundCheck(const TiledMap *map) {
  const Vector2 probe = {position.x + sprite.GetSize().x / 2.0f,
                         position.y + 1.0f};
...
const Rectangle Player::GetCollisionRect() const {
  return Rectangle{position.x, position.y, 32.0f, 32.0f};
}

void Player::UpdatePlayerPosition(const Vector2 newPosition) {
  position = newPosition;
}

void Player::SnapPlayerToGround() {
  position.y = std::floor((position.y + sprite.GetSize().y) / 16.0f) * 16.0f -
               sprite.GetSize().y;
}
